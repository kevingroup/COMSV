/*
  ...
  Indel caller of COMSV pipeline
*/
...
void readSourceFile(){
    ...
    while (fscanf(inputAlignmentFile, "%s", mmid) == 1){
        ...
        LL aliSt = min(opticalMap1[numberOfOpticalMap].optStart, opticalMap1[numberOfOpticalMap].optEnd)-1;
        LL aliEd = max(opticalMap1[numberOfOpticalMap].optStart, opticalMap1[numberOfOpticalMap].optEnd);
        LL aliReg = opticalMap1[numberOfOpticalMap].position[aliEd] - opticalMap1[numberOfOpticalMap].position[aliSt];
        double fltRatio = 0.1;
        LL fltLen = 50000;
        if ((aliReg*1.0/totLen < fltRatio) && (aliReg < fltLen)){
            rem_cnt++;
            continue;
        }
        ...
        for (int k = 0; k < highDens.size(); k++){
            if (highDens[k].chr == opticalMap1[numberOfOpticalMap].chrId && ((opticalMap1[numberOfOpticalMap].refStart - highDens[k].stop)*(opticalMap1[numberOfOpticalMap].refEnd - highDens[k].start))<=0){
                isIn = true;
                break;
            }
        }
        if (isIn)rem_cnt++;
        else numberOfOpticalMap++;
...
void setDefault() {
    ...
    minIndelSize = 2000;
    minSVDiffSize = 5000;
    minClsSI = 3; // 0.2 for cancer samples, 0.3 for normal samples
    minClsSD = 3; // 0.2 for cancer samples, 0.3 for normal samples
    minVar = 5; // minimum number of molecules having variation distance to be considered
    minClsSizRat = 0.2; // 0.2 for cancer samples, 0.3 for normal samples
    minSVSuppRat = 0.1;
    minIndelRatio = 0.05;
    inputFlag = 0507;
}
...
void detectByDistancePair(int maxClsNum, int fltLv, bool cancer){
    ...
    //change in april 19
    int extraFltLv = 0;
    if (cancer)extraFltLv=1;
    if (fltLv>=3+extraFltLv)minClsSizRat = 0.35;
    else if (fltLv>=2+extraFltLv)minClsSizRat = 0.3;
    else if (fltLv==1+extraFltLv)minClsSizRat = 0.25;
    else minClsSizRat = 0.2;
    ...
    int btClsSize = ceil(log2(numberOfSupportIndelMolecule*1.0)); // minimum class size is max(log2(minSupport),size*20%)
...
bool postProcess(vector<int>& finalIdx, vector<LL>& finalMap, vector<double> finalDat, LL refDist, OMSV & resSV){
    ...
    int minClsSize = 3; // minimum class size is 3
...
double parseHitEnum(){
    ...
    int totN = 0;
    int fltLv = 0;
    for (LL i = 0; i <= mxNum; i++){
        if (totN < SvRatioVec.size()*0.9){
            totN += distSVCov[i];
            fltLv = i;
        }
    }
...
void correctOverlapVariant(){
    ...
    for (int i = 0; i < tempRealCnt; i++){
        ...
        if (grpSVs.size() == 1){
            grpSVs[0].start_i = grpSVs[0].start;
            grpSVs[0].end_i = grpSVs[0].end;
            grpSVs[0].start_u = grpSVs[0].start;
            grpSVs[0].end_u = grpSVs[0].end;
            ...
            LL st_int = grpSVs[0].start;
            LL st_uni = grpSVs[0].start;
            LL ed_int = grpSVs[0].end;
            LL ed_uni = grpSVs[0].end;
            bool rep = false;
            for (int s = 1; s < grpSVs.size(); s++){
                st_int = max(st_int,grpSVs[s].start);
                st_uni = min(st_uni,grpSVs[s].start);
                ed_int = min(ed_int,grpSVs[s].end);
                ed_uni = max(ed_uni,grpSVs[s].end);
                rep = false;
            ...
            grpSVs[flIdx].start_i = st_int;
            grpSVs[flIdx].start_u = st_uni;
            grpSVs[flIdx].end_i = ed_int;
            grpSVs[flIdx].end_u = ed_uni;
            if (grpSVs[flIdx].start_i > grpSVs[flIdx].end_i) {grpSVs[flIdx].end_i = -1;grpSVs[flIdx].start_i=-1;}
...
void outputVariantResult(int argv, char* argc[], char* outputFolder,int chrID){
    ...
    fprintf(outputVariantResultFile, "\n#Chr\tStart_conf\tStop_conf\tStart_int\tStop_int\tStart_uni\tStop_uni\tType\tBlank\tSize1(for diploid)\tID\tSize2(for diploid)\tZygosity(Sizes for >2 haplotypes)\tConfidence\tScore\tCoverage\tSupport\n");
    LL variantCallId = 1;
    ...
    for (LL i=0; i<allSVs.size(); i++){
        if (allSVs[i].type!="Reference"){
            ...
            fprintf(outputVariantResultFile, "%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%s\t%d\t%.1f\t%d\tsizeChange=%.1f\t%s\t%g\t%g\t%g\t%g\n", allSVs[i].chr, allSVs[i].start, allSVs[i].end, allSVs[i].start_i, allSVs[i].end_i, allSVs[i].start_u, allSVs[i].end_u, allSVs[i].type.c_str(), 0,  allSVs[i].difSiz1, 0, allSVs[i].difSiz2, allSVs[i].zyg.c_str(), allSVs[i].score, allSVs[i].score,allSVs[i].coverage, allSVs[i].support);
            numberOfSupportedSV++;
        }
    }
...
int main(int argv, char *argc[]){
    ...
    for (LL tempChrId = 0; tempChrId < (LL)listOfChromosome.size(); tempChrId++){
        ...
        int fltLv = parseHitEnum();
        printf("Distance Pair Count-2: %lld\n", distancePairCount);
        detectByDistancePair(maxClsNum,fltLv,cM);
