/*
  ...
  tool to deduplicate a list of indels
*/
#include "Header.h"
#include <float.h>
#include <unistd.h>
        ...
        if (grpSVs.size() == 1){
            ...
            for (int s = 1; s < grpSVs.size(); s++){
                ...
                if (!rep){
                    ...
                    tpEle.selIdx = s;
                    tpEle.num = 1;
		    memset(tpEle.extra,0,sizeof(tpEle.extra));
		    strcpy(tpEle.extra, grpSVs[s].extra);
            ...
            for (int s = 0; s < grpSVs.size(); s++){
                for (int t = 0; t < triVec.size(); t++){
                    if (triVec[t].zyg == grpSVs[s].zyg && triVec[t].type == grpSVs[s].type && strstr(grpSVs[s].extra,"Single-Molecule-Alignment")){
                        ...
                        if ( min(max(abs(grpSVs[s].difSiz1),abs(grpSVs[s].difSiz2)),max(abs(grpSVs[fidx].difSiz1),abs(grpSVs[fidx].difSiz2))) >= min(max(abs(grpSVs[s].difSiz1),abs(grpSVs[s].difSiz2)),max(abs(grpSVs[fidx].difSiz1),abs(grpSVs[fidx].difSiz2)))*0.8 ){
                            if (grpSVs[s].end - grpSVs[s].start < grpSVs[sidx].end - grpSVs[sidx].start){
                                triVec[t].selIdx = s;
				    memset(tpEle.extra,0,sizeof(tpEle.extra));
	        		    strcpy(triVec[t].extra, grpSVs[s].extra);
                            }
                        }
                        break;
                    }
            ...
            double maxCov = 0;
	    char extras[10000] = "Contig-Alignment";
            int flIdx = -1;
            for (int s = 0; s < triVec.size(); s++){
                if (triVec[s].num == maxN){
                    if (flIdx == -1 || (maxCov < triVec[s].coverage) || (strstr(triVec[s].extra,"Single-Molecule-Alignment")&&strstr(extras,"Contig-Alignment"))){
                        maxCov = triVec[s].coverage;
                        flIdx = triVec[s].selIdx;
			memset(extras,0,sizeof(extras));
			strcpy(extras,triVec[s].extra);
                    }
                }
            }
...
void printFile(char* outF)
{
        ...
        char tp[1000];
        fprintf(outputF,"#chr\tstart\tend\ttype\ttp1\ttp2\ttp3\tsize\tzygosity\tc10\tc11\tc12\tc13\n");
