...
{//1 is the SV bed, 2 is the FS bed
	...
	int typSame = 0;
	if (type1[0]==type2[0]&&type1[1]==type2[1]&&type1[2]==type2[2]) typSame += 20;
...
void readFS(char* inputFS)
{
	...
	char temp2[100];
	char temp3[1000];
	while(fscanf(inputFragileSite,"%s\t%lld\t%lld",tempSV.chr, &tempSV.start, &tempSV.end)==3)
	{
...
void readSVBed(char* inputSV)
{
	...
	char temp2[100];
	char temp3[1000];
        while(fscanf(inputBed,"%s\t%lld\t%lld",tempSV.chr, &tempSV.start, &tempSV.end)==3)
        {
...
void printFile()
{
	...
	{
		fprintf(outputF,"%s\t%lld\t%lld\t%d\n",resList[i].chr, resList[i].startSV, resList[i].endSV,resList[i].score);
	}
}
